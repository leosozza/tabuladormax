...
DROP FUNCTION IF EXISTS public.get_scouter_leads_simple(text, text, text, text, integer);
...
BEGIN
  ...
  -- Return paginated results with template stats via efficient LEFT JOIN
  RETURN QUERY
  ...
  -- Pre-aggregate template stats using a single pass
  template_stats AS (
    SELECT 
      w.phone_number,
      COUNT(*) AS send_count,
