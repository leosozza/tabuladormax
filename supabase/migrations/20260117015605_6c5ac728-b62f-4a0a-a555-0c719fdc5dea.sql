...
-- Corrigir RPC para incluir fallback por telefone quando bitrix_id estiver nulo
...
BEGIN
  ...
  lead_with_whatsapp AS (
    ...
    LEFT JOIN LATERAL (
      SELECT 
        COUNT(*) AS send_count,
        -- Buscar último status: primeiro por bitrix_id, depois por telefone se bitrix_id for nulo
        (
          SELECT w.status 
          ...
          WHERE w.direction = 'outbound' 
            ...
            AND (
              ...
              OR (w.bitrix_id IS NULL AND w.phone_number = fl.phone_normalized)
            )
          ORDER BY 
        ...
        -- Buscar último erro: primeiro por bitrix_id, depois por telefone se bitrix_id for nulo
        (
...
-- Criar índices para melhorar performance do fallback por telefone
CREATE INDEX IF NOT EXISTS idx_whatsapp_messages_phone_normalized 
ON whatsapp_messages (phone_number) 
WHERE direction = 'outbound' AND message_type = 'template';

CREATE INDEX IF NOT EXISTS idx_whatsapp_messages_bitrix_id_template 
ON whatsapp_messages (bitrix_id) 
WHERE direction = 'outbound' AND message_type = 'template';
