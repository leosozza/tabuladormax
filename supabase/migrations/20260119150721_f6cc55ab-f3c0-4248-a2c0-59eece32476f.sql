-- Fix admin WhatsApp conversations RPC to never block loading due to missing lead/operator data
...
AS $$
  ...
  -- IMPORTANT: lead is optional; join must NEVER block conversations from loading
  LEFT JOIN public.leads l
    ON (s.bitrix_id ~ '^\d+$' AND l.id = (s.bitrix_id::BIGINT))
  -- operator is optional; used only for identification
  ...
  WHERE
    ...
    -- Window filter (24h window from last inbound)
    AND (
    ...
    -- Etapa filter (only applies if we have lead data)
    AND (
