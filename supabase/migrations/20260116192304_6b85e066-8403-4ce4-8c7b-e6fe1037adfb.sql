-- Update RPC to also show assigned operator from lead when no human operator message exists
...
BEGIN
  ...
  last_operators AS (
    ...
    WHERE wm.direction = 'outbound'
      ...
      AND LOWER(wm.sender_name) NOT LIKE '%bitrix%'
    ORDER BY COALESCE(wm.phone_number, wm.bitrix_id), wm.created_at DESC
  ),
  conversations AS (
    SELECT 
      cs.phone_number,
      cs.bitrix_id,
      COALESCE(l.name, cs.phone_number, cs.bitrix_id) as lead_name,
      l.id as lead_id,
      cs.last_message_at,
      ...
      cs.unread_count,
      cs.total_messages,
      -- Prioritize last message operator, fallback to assigned lead operator
      COALESCE(lo.operator_name, assigned_op.name) as last_operator_name,
      COALESCE(lo.operator_photo_url, assigned_op.photo_url) as last_operator_photo_url
    ...
    -- Fallback: get assigned operator from lead's bitrix_telemarketing_id
    LEFT JOIN telemarketing_operators assigned_op ON assigned_op.bitrix_id = l.bitrix_telemarketing_id
  )
  SELECT 
    c.phone_number,
    c.bitrix_id,
