...
-- Recreate with consolidated deal status logic and deal_count
...
BEGIN
  ...
  SELECT 
    ...
    -- Consolidated deal status: won > open > lost
    CASE 
  ...
  LEFT JOIN leads l ON l.id = CASE 
    ...
    ELSE NULL 
  END
  -- Subquery to calculate consolidated status from ALL deals for this lead
  LEFT JOIN LATERAL (
    SELECT 
      ...
      -- Get the most recent deal's stage_id and category_id for reference
  ...
  WHERE 
    ...
    -- Deal status filter with consolidated logic
...
-- Update count function with the same consolidated logic
