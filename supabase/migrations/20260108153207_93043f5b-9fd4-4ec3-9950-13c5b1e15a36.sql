-- Fix: não é possível alterar o RETURNS TABLE via CREATE OR REPLACE; precisamos dropar e recriar
DROP FUNCTION IF EXISTS public.get_telemarketing_whatsapp_messages(integer, text, bigint, integer[], integer);

CREATE FUNCTION public.get_telemarketing_whatsapp_messages(
...
BEGIN
  ...
  -- Resolver lead interno + bitrix_id do lead
  IF p_lead_id IS NOT NULL THEN
    v_lead_internal_id := p_lead_id;

    SELECT l.bitrix_id::text
      INTO v_lead_bitrix_id
    FROM leads l
    ...
    IF v_lead_bitrix_id IS NULL THEN
      RETURN;
    END IF;
  ...
  WHERE (
    ...
    OR (v_lead_bitrix_id IS NOT NULL AND m.bitrix_id = v_lead_bitrix_id)
  )
...
-- Ajuste no mark as read para vincular corretamente pelo bitrix_id do lead
...
BEGIN
  ...
  WITH accessible_messages AS (
    SELECT m.id
    FROM whatsapp_messages m
    JOIN leads l ON l.bitrix_id::text = m.bitrix_id
