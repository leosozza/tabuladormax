-- Fix: get_my_invited_conversations_full - change stage_id to etapa
-- Also add security check to prevent unauthorized access
...
BEGIN
  ...
  SELECT 
    ...
    l.etapa as lead_etapa,  -- FIXED: was l.stage_id which doesn't exist
  ...
  WHERE p.operator_id = p_operator_id
    AND p.resolved_at IS NULL  -- Only show active invitations
