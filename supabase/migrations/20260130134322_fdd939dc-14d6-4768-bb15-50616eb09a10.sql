-- Ensure the text[]/text overloads (still present) are also fixed, since PostgREST may resolve to them.
...
DECLARE
  v_tag_filter uuid[];
  v_operator_filter uuid;
BEGIN
  -- Safe casts
  IF p_tag_filter IS NOT NULL AND array_length(p_tag_filter, 1) IS NOT NULL THEN
    SELECT array_agg(x::uuid)
      INTO v_tag_filter
    FROM unnest(p_tag_filter) AS x
    WHERE x ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$';
  END IF;

  IF p_operator_filter IS NOT NULL AND p_operator_filter ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' THEN
    v_operator_filter := p_operator_filter::uuid;
  END IF;
  ...
  SELECT
    ...
    CASE
      WHEN deal_summary.latest_stage_id LIKE '%:WON' THEN 'won'
      WHEN deal_summary.latest_stage_id LIKE '%:LOSE' THEN 'lost'
      WHEN deal_summary.latest_stage_id IS NOT NULL THEN 'open'
      ELSE NULL
    ...
    deal_summary.contract_number::text AS contract_number,
  ...
  WHERE
    (p_search IS NULL OR p_search = '' OR
      ...
      COALESCE(l.name, l.nome_modelo) ILIKE '%' || p_search || '%' OR
      COALESCE(deal_summary.contract_number, '') ILIKE '%' || p_search || '%' OR
      COALESCE(deal_summary.maxsystem_id, l.maxsystem_id_ficha, '') ILIKE '%' || p_search || '%')
