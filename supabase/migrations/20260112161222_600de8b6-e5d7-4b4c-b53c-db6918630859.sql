-- Drop and recreate the RPC to fix UUID comparison issue
DROP FUNCTION IF EXISTS get_scouter_leads_simple(text, timestamptz, timestamptz, text, text);
...
BEGIN
  ...
  SELECT
    ...
    -- Contagem de templates já enviados para controle de reenvio
  ...
  WHERE l.scouter = p_scouter_name
    ...
    -- FIX: Cast p_project_id to UUID for proper comparison
