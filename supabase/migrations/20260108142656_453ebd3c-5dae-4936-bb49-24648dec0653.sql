-- RPC para buscar conversas de WhatsApp para telemarketing
-- SECURITY DEFINER permite acesso sem auth.uid()
-- Validação é feita pelo bitrix_id do operador
...
BEGIN
  -- Determinar quais operadores buscar
  ...
  WITH lead_data AS (
    SELECT 
      ...
      COALESCE(l.nome_modelo, l.nome, 'Lead ' || l.id) AS lead_name,
      COALESCE(
        NULLIF(l.telefone_principal, ''),
        NULLIF(l.telefone_secundario, ''),
        NULLIF(l.telefone_modelo, '')
      ) AS phone_number,
      l.foto_url AS photo_url,
      l.bitrix_telemarketing_id,
      t.nome AS telemarketing_name
    FROM leads l
    ...
    WHERE l.bitrix_telemarketing_id = ANY(v_operator_ids)
      AND (
        ...
        OR l.telefone_principal ILIKE '%' || p_search || '%'
      )
  ),
  -- Buscar telefones de mensagens para leads sem telefone
  phone_fallback AS (
  ...
  message_stats AS (
    SELECT 
      ...
      COUNT(*) FILTER (WHERE wm.direction = 'inbound' AND wm.is_read = false) AS unread_count,
...
-- Permitir acesso para authenticated e anon (telemarketing usa chave, não auth)
GRANT EXECUTE ON FUNCTION public.get_telemarketing_conversations TO authenticated, anon;

COMMENT ON FUNCTION public.get_telemarketing_conversations IS 
'Busca conversas de WhatsApp para operadores de telemarketing. 
Não depende de auth.uid() - validação por bitrix_id do operador.';