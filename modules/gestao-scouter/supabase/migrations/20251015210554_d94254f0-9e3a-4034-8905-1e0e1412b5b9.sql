-- ============================================
-- FASE 2: SCHEMA COMPLETO DO BANCO COMPARTILHADO
-- Sistema integrado: Gestão Scouter + TabuladorMax
-- ============================================
...
CREATE INDEX idx_fichas_scouter ON fichas(scouter);
CREATE INDEX idx_fichas_projeto ON fichas(projeto);
CREATE INDEX idx_fichas_tabulacao ON fichas(tabulacao);
CREATE INDEX idx_fichas_criado ON fichas(criado);
CREATE INDEX idx_fichas_bitrix_id ON fichas(bitrix_id);
CREATE INDEX idx_fichas_telemarketing_user ON fichas(telemarketing_user_id);
CREATE INDEX idx_fichas_scouter_user ON fichas(scouter_user_id);
...
CREATE TABLE public.scouter_locations (
  id SERIAL PRIMARY KEY,
...
CREATE INDEX idx_scouter_locations_timestamp ON scouter_locations(timestamp);
CREATE INDEX idx_scouter_locations_scouter ON scouter_locations(scouter_id);

-- Pagamentos de ajuda de custo
CREATE TABLE public.scouter_payments (
  id SERIAL PRIMARY KEY,
  scouter_id INTEGER REFERENCES scouter_profiles(id) ON DELETE CASCADE,
...
CREATE TABLE public.scouter_metrics (
  id SERIAL PRIMARY KEY,
  scouter_id INTEGER REFERENCES scouter_profiles(id) ON DELETE CASCADE,
  periodo TEXT NOT NULL,
...
CREATE TABLE public.telemarketing_agents (
  id SERIAL PRIMARY KEY,
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE UNIQUE,
  nome TEXT NOT NULL,
  telefone TEXT,
...
CREATE INDEX idx_call_logs_ficha ON call_logs(ficha_id);
...
CREATE TABLE public.telemarketing_metrics (
  id SERIAL PRIMARY KEY,
  agent_id INTEGER REFERENCES telemarketing_agents(id),
  periodo TEXT NOT NULL,
...
-- Inserir permissões padrão para Gestão Scouter
