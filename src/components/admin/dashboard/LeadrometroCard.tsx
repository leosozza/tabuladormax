...
export function LeadrometroCard({ dateFilter, sourceFilter = 'all' }: LeadrometroCardProps) {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['leadrometro', dateFilter.startDate?.toISOString(), dateFilter.endDate?.toISOString(), sourceFilter],
    queryFn: async () => {
      const startDate = dateFilter.startDate.toISOString();
      const endDate = dateFilter.endDate.toISOString();

      // Usar RPC para evitar limite de 1000 linhas e aplicar sourceFilter
      const { data, error } = await supabase.rpc('get_leadrometro_stats', {
      ...
      // RPC retorna array com um único registro
      const stats = data?.[0] || {
        total: 0,
        com_foto: 0,
        confirmados: 0,
      ...
      return {
        ...
        comFoto: Number(stats.com_foto) || 0,
        confirmados: Number(stats.confirmados) || 0,
        agendados: Number(stats.agendados) || 0,
        convertidos: Number(stats.convertidos) || 0,
        emAnalise: Number(stats.em_analise) || 0
      };
    },
                ...
                <div key={key} className="space-y-2">
                  <div className="flex justify-between">
                    ...
                    <span className="text-sm text-muted-foreground">
                      {tempWeights[key as keyof Weights]}%
                    </span>
                  </div>
                  <Slider
