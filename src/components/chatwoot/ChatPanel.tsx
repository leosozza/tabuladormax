...
export function ChatPanel({
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data: contact
      } = await supabase.from('chatwoot_contacts').select('bitrix_id, last_customer_message_at').eq('conversation_id', conversationId).maybeSingle();
  ...
  return <div className="flex h-full flex-col bg-background">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col min-h-0">
        ...
        <TabsContent value="messages" className="flex-1 flex-col min-h-0 mt-0 data-[state=active]:flex">
          <ScrollArea className="flex-1 p-4">
            <div className="space-y-4" ref={scrollRef}>
              {loading && messages.length === 0 ? <div className="flex justify-center py-8">
                  <div className="text-muted-foreground">Carregando mensagens...</div>
                </div> : messages.length === 0 ? <div className="flex flex-col items-center justify-center py-8 text-center">
                  <MessageSquare className="w-12 h-12 text-muted-foreground mb-2" />
                  <p className="text-muted-foreground">Nenhuma mensagem ainda</p>
                </div> : messages.map(msg => <div key={msg.id} className={`flex ${msg.message_type === 1 ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[70%] rounded-lg px-4 py-2 ${msg.message_type === 1 ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}>
                      <div className="text-sm font-medium mb-1">
                        {msg.sender?.name || (msg.message_type === 1 ? 'Você' : 'Cliente')}
                      </div>
                      <div className="whitespace-pre-wrap">{msg.content}</div>
                      {msg.attachments && msg.attachments.length > 0 && <div className="mt-2 space-y-2">
                          {msg.attachments.map((att, idx) => <a key={idx} href={att.data_url} target="_blank" rel="noopener noreferrer" className="text-xs underline">
                              📎 {att.file_type}
                            </a>)}
                        </div>}
          ...
          <div className="border-t p-4 bg-card">
            {isWindowOpen ? <>
                <div className="flex gap-2 items-end">
                  <Textarea placeholder="Digite sua mensagem... (Ctrl+Enter para enviar)" value={messageInput} onChange={e => setMessageInput(e.target.value)} onKeyDown={handleKeyPress} disabled={sending} className="min-h-[80px] max-h-[160px] resize-none" rows={3} />
                  <Button onClick={handleSendMessage} disabled={sending || !messageInput.trim()} size="icon" className="flex-shrink-0">
        ...
        <TabsContent value="templates" className="flex-1 flex-col min-h-0 mt-0 data-[state=active]:flex">
          <TemplateSelector onSendTemplate={sendTemplate} disabled={sending} />
        </TabsContent>
      </Tabs>
    </div>;
}