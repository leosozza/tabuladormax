...
export const useLeadColumnConfig = () => {
  ...
  const toggleColumn = (key: string) => {
    setVisibleColumns(prev => {
      if (prev.includes(key)) {
        // Don't allow removing if at minimum or if it's 'name' (mandatory)
        if (prev.length <= MIN_COLUMNS || key === 'name') {
          return prev;
        }
  ...
  const clearAll = () => {
    setVisibleColumns(['name', 'scouter', 'projetos']); // Minimum required
  };
  ...
  const canToggle = (key: string) => {
    if (key === 'name') return false; // Name is mandatory
