...
const PreCadastro = () => {
  ...
  const [leadData, setLeadData] = useState<LeadData>({
    nomeResponsavel: "", estadoCivil: "", telefone: "", cidade: "", estado: "",
    nomeModelo: "", dataNascimento: "", sexo: "", altura: "", peso: "",
    busto: "", cintura: "", quadril: "", corPele: "", corCabelo: "",
    corOlhos: "", tipoCabelo: "", tamanhoCamisa: "", tamanhoCalca: "", tamanhoSapato: "",
    instagram: "", instagramSeguidores: "", facebook: "", facebookSeguidores: "",
    youtube: "", youtubeSeguidores: "", tiktok: "", tiktokSeguidores: "",
    tiposModelo: [], cursos: [], habilidades: [], caracteristicas: []
  });
    ...
    input.onchange = async (e) => {
      ...
      if (file) {
        try {
          ...
          const { error } = await supabase.storage.from('lead-photos').upload(filePath, file);
          if (error) throw error;

          const { data: { publicUrl } } = supabase.storage.from('lead-photos').getPublicUrl(filePath);
      ...
      try {
        await supabase.functions.invoke('bitrix-entity-update', {
          body: {
            ...
            fields: {
              NAME: leadData.nomeModelo,
              PHONE: [{ VALUE: leadData.telefone, VALUE_TYPE: "MOBILE" }],
              ...updateData.raw
            }
          }
        });
    ...
    <div className="min-h-screen bg-background">
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8 text-center">
          <h1 className="text-3xl font-bold">Atualização de Cadastro</h1>
          <p className="text-muted-foreground">Complete suas informações</p>
        </div>
          ...
          <div className="lg:col-span-1 space-y-4">
            ...
            <div className="grid grid-cols-5 gap-2">
              {images.map((image, index) => (
                <button key={index} onClick={() => setActiveImageIndex(index)} className={`aspect-square rounded-lg overflow-hidden border-2 transition-all ${activeImageIndex === index ? "border-primary shadow-glow" : "border-transparent hover:border-border"}`}>
                  <img src={image} alt={`Foto ${index + 1}`} className="w-full h-full object-cover" />
                </button>
              ))}
              {images.length < 10 && (
                <button onClick={handleAddPhoto} className="aspect-square rounded-lg border-2 border-dashed border-border hover:border-primary transition-all flex items-center justify-center group">
