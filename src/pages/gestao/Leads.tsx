...
function GestaoLeadsContent({
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const mandatoryFields = ['id', 'name', 'criado', 'nome_modelo', 'scouter', 'photo_url', 'projeto_comercial'];
      ...
      const fieldsToSelect = Array.from(new Set([...mandatoryFields, ...directFields]));
      ...
      const queryBuilder = supabase.from("leads").select(selectClause).order("criado", {
        ascending: false
      }).limit(100);
      ...
      queryBuilder.gte("criado", filters.dateFilter.startDate.toISOString()).lte("criado", filters.dateFilter.endDate.toISOString());
  ...
  const visibleFields = visibleColumns.map(key => allFields?.find(field => field.key === key)).filter((field): field is NonNullable<typeof field> => field !== undefined);
  ...
  const renderCellValue = (field: any, value: any, lead: any) => {
    ...
    let formattedValue = field.formatter ? field.formatter(displayValue, lead) : displayValue || "-";
    ...
    if (field.bitrixField && field.bitrixFieldType && ['enumeration', 'crm_status', 'crm_stage'].includes(field.bitrixFieldType) && displayValue != null && displayValue !== '') {
  ...
  return <GestaoPageLayout title="Gestão de Leads" actions={
    <div className="flex gap-2 items-center">
      {searchExpanded ? (
        ...
        <div className="flex gap-2 items-center animate-in slide-in-from-right-5">
          <div className="relative">
            ...
            <Input placeholder="Nome, ID ou modelo..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-9 pr-9 h-8 w-48 md:w-64" autoFocus />
      ...
      <div className="flex flex-col gap-4">
        ...
        {isSelectionMode && <div className="lg:hidden bg-blue-100 border border-blue-300 rounded-lg p-3 flex items-center justify-between animate-in slide-in-from-top-2">
            <div className="flex items-center gap-2">
              <div className="h-2 w-2 bg-blue-600 rounded-full animate-pulse" />
              <span className="text-sm font-medium text-blue-900">
                {selectedLeadIds.size > 0 ? `${selectedLeadIds.size} lead${selectedLeadIds.size > 1 ? 's' : ''} selecionado${selectedLeadIds.size > 1 ? 's' : ''}` : 'Modo de seleção ativo'}
              </span>
            </div>
            <Button variant="ghost" size="sm" onClick={toggleSelectionMode} className="h-7 text-blue-700 hover:text-blue-900 hover:bg-blue-200">
              Sair
            </Button>
          </div>}
        ...
        <div className="flex-1 overflow-auto">
          <div className="rounded-md border">
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead className={cn("w-8 p-1 text-center h-8", !isSelectionMode && "hidden lg:table-cell")}>
                    <Checkbox className="h-2 w-2" checked={selectedLeadIds.size === leads?.length && leads.length > 0} onCheckedChange={toggleSelectAll} aria-label="Selecionar todos" />
                  </TableHead>
              ...
              <TableBody>
                {isLoading ? <TableRow>
                  ...
                  </TableRow> : !leads?.length ? <TableRow>
                    <TableCell colSpan={visibleFields.length + 1} className="text-center">
                      Nenhum lead encontrado
                    </TableCell>
                  </TableRow> : leads?.map(lead => <TableRow key={lead.id} className={cn("cursor-pointer hover:bg-muted/50 transition-colors", selectedLeadIds.has(lead.id) && isSelectionMode && "bg-blue-50 hover:bg-blue-100")} onTouchStart={e => {
                e.preventDefault();
                handleLongPressStart(lead.id);
              }} onTouchEnd={handleLongPressEnd} onTouchMove={handleLongPressEnd} onMouseDown={() => handleLongPressStart(lead.id)} onMouseUp={handleLongPressEnd} onMouseLeave={handleLongPressEnd}>
                      <TableCell onClick={e => e.stopPropagation()} className={cn("w-8 p-1 text-center", !isSelectionMode && "hidden lg:table-cell")}>
                        <Checkbox className="h-2 w-2" checked={selectedLeadIds.has(lead.id)} onCheckedChange={() => toggleSelectLead(lead.id)} aria-label={`Selecionar lead ${lead.id}`} />
                      </TableCell>
                      {visibleFields.map(field => {
                  const value = lead[field.key];
                  return <TableCell key={field.key} className={field.key === 'name' ? 'font-medium' : ''} onClick={() => handleLeadClick(lead)}>
      ...
      {isAnalysisMode && analysisLeads.length > 0 && <LeadAnalysisModal lead={analysisLeads[currentAnalysisIndex]} open={isAnalysisMode} onOpenChange={open => {
      if (!open) {
        setIsAnalysisMode(false);
        setSelectedLeadIds(new Set());
      }
    }} onSwipe={direction => {
      if (direction === 'left') handleReject();else if (direction === 'right') handleApprove();else if (direction === 'up') handleSuperApprove();else if (direction === 'down') handleSkip();
    }} onApprove={handleApprove} onReject={handleReject} onSuperApprove={handleSuperApprove} onSkip={handleSkip} onUndo={handleUndo} canUndo={isUndoAvailable} disabled={isProcessingAction} currentIndex={currentAnalysisIndex} totalLeads={analysisLeads.length} />}
      
      <TinderCardConfigModal open={configModalOpen} onOpenChange={setConfigModalOpen} />
    </GestaoPageLayout>;
}
